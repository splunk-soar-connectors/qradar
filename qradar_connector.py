...
class QradarConnector(BaseConnector):
    ...
    def _get_json_error_message(self, response, action_result):
        ...
        message = f"Error from server. Status Code: {response.status_code} Error Code: {error_code}, Error Message: {error_message}, Error Description: {error_description}"
        ...
        if self.get_action_identifier() == "test_asset_connectivity":
            ...
            if "Authorization" in headers:
                try:
                    ...
                    error_message = self._get_error_message_from_exception(e)
                    result.set_status(
                        phantom.APP_ERROR, f"{QRADAR_ERROR_REST_API_CALL_FAILED}. {QRADAR_BASIC_AUTH_ERROR_MESSAGE}", error_message
                    )
                    return r
            ...
            if sec_token_header:
                ...
                try:
                    ...
                    error_message = self._get_error_message_from_exception(e)
                    result.set_status(
                        phantom.APP_ERROR, f"{QRADAR_ERROR_REST_API_CALL_FAILED}. {QRADAR_AUTH_TOKEN_ERROR_MESSAGE}", error_message
                    )
                    return r
        else:
            try:
    ...
    def _set_auth(self, config):
        ...
        if username and password:
            try:
                ...
                auth_string = "Basic {}".format(base64.b64encode(user_pass.encode("ascii")).decode("ascii"))

                self._auth["Authorization"] = auth_string
            except Exception:
                error_message = (
                    "Error occurred while generating authorization headers. Please check the credentials in the asset configuration parameters."
                )
                self.set_status(phantom.APP_ERROR, error_message)
                return phantom.APP_ERROR
    ...
    def initialize(self):
        ...
        ret_val, self._events_ingest_start_time = self._validate_integer(
            self, self._events_ingest_start_time, QRADAR_EVENTS_INGEST_START_TIME_KEY
        )
        ...
        if phantom.is_fail(ret_val):
            return self.get_status()
        ret_val, self._offense_ingest_start_time = self._validate_integer(
            self, self._offense_ingest_start_time, QRADAR_OFFENSE_INGEST_START_TIME, True
        )
        if phantom.is_fail(ret_val):
            return self.get_status()
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.set_status(phantom.APP_ERROR, f"{cef_map_error_message}. Error message: {error_message}")
            return phantom.APP_ERROR

        self._on_poll_action_result = None

        # Base URL
        self._base_url = f"https://{self._server}/api/"
    ...
    def _get_artifact(self, event, container_id):
        ...
        get_ph_severity = lambda x: phantom.SEVERITY_LOW if x <= 3 else (phantom.SEVERITY_MEDIUM if x <= 7 else phantom.SEVERITY_HIGH)
    ...
    def _test_connectivity(self, param):
        ...
        if phantom.is_fail(action_result.get_status()):
            self.save_progress(f"Error occurred while connecting QRadar instance with Server Hostname | IP : {self._server}")
        ...
        if response.status_code != 200:
            ...
            if "json" in response.headers.get("Content-Type", ""):
                ...
                status_message = f"{QRADAR_ERROR_CONNECTIVITY_TEST}. {QRADAR_MESSAGE_CHECK_CREDENTIALS}. HTTP status_code: {response.status_code}, reason: {response.reason}"
    ...
    def _createfilter(self, param, action_result):
            ...
            for x in offense_ids_list:
                try:
                    if len(x.strip()) > 0 and int(x.strip()) >= 0:
                        interim_offense_ids_list.append(f"{int(x.strip())}")
                except Exception:
        ...
        if len(offense_ids_list) > 0:
            ...
            try:
                ...
                self.save_progress(f"Initial time for fetching the offenses is : {start_time}")
                ...
                if isinstance(start_time, str):
                    if start_time.isdigit():
                        ...
                        if start_time < 0:
                            return (
                                action_result.set_status(
                                    phantom.APP_ERROR,
                                    QRADAR_ERROR_INVALID_TIME.format(
                                        num_type="positive",
                                        field_name="Alternative initial ingestion time",
                                        field_location="asset configuration parameter",
                                    ),
                                ),
                                None,
                                None,
                        ...
                        try:
                            ...
                            self.debug_print(
                                "Checking for negative integer values failed "
                                "for 'Alternative initial ingestion time' asset configuration parameter"
                            )
                            self.save_progress(f"The 'initial time' for fetching the offenses is derived from string: {start_time}")
                ...
                if isinstance(end_time, str):
                    if end_time.isdigit():
                        ...
                        if end_time <= 0:
                            return (
                                action_result.set_status(
                                    phantom.APP_ERROR,
                                    QRADAR_ERROR_INVALID_TIME.format(
                                        num_type="non-zero positive", field_name="end_time", field_location="parameter"
                                    ),
                                ),
                                None,
                                None,
                        ...
                        try:
                            ...
                            self.save_progress(f"The 'end time' for fetching the offenses is derived from string: {end_time}")
            ...
            try:
                self.save_progress(f"start_time: {self._utcctime(start_time)}")
                self.save_progress(f"end_time:   {self._utcctime(end_time)}")
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                self.debug_print(
                    f"For alternate ingestion workflow of fetching offenses, \
                    provided time is invalid. Error: {error_message}"
                )
            ...
            if self._is_on_poll and self._offense_ingest_start_time > 0:
                self.save_progress(f"Original start_time is: {start_time}")
                start_time = start_time - self._offense_ingest_start_time * 60 * 1000
                self.save_progress(f"Back-dating the offense start time by {self._offense_ingest_start_time} minutes")
                self.save_progress(f"The modified start_time is: {start_time}")
            ...
            if self._time_field == "last_updated_time":
                reqfilter = f"(last_updated_time >= {start_time} and last_updated_time <= {end_time})"
            elif self._time_field == "either":
                reqfilter = f"((start_time >= {start_time} and start_time <= {end_time}) or (last_updated_time >= {start_time} and last_updated_time <= {end_time}))"
            else:
                self._time_field = "start_time"
                reqfilter = f"(start_time >= {start_time} and start_time <= {end_time})"

            try:
                self.save_progress(
                    f"Applying time range between [{self._utcctime(start_time)} -> {self._utcctime(end_time)}] inclusive (total minutes {(end_time - start_time) / (1000 * 60)})"
                )
            except Exception as e:
                error_message = self._get_error_message_from_exception(e)
                self.debug_print(f"Provided time is invalid. Error: {error_message}")

        # last requirement, are we listing only opened offenses?
        if self._ingest_only_open:
            reqfilter += " and status=OPEN"

        self.save_progress(f"For alternate ingestion workflow of fetching offenses, using filter: {reqfilter}")
    ...
    def _alt_list_offenses(self, param, action_result=None):
        ...
        if ingestion_order == "oldest first":
            ...
            reqparams["sort"] = f"+{self._time_field}"
        ...
        while True:
            ...
            reqheaders["Range"] = f"items={start_index}-{end_index}"
            self.save_progress(f"Retrieving index: {start_index} -> {end_index}")
        ...
        if len(offenses) > 0:
            ...
            for offense in offenses:
                try:
                    self.save_progress(
                        "Queuing offense id: {} start_time({}, {}) last_updated_time({}, {})".format(
                            offense["id"],
                            offense["start_time"],
                            self._utcctime(offense["start_time"]),
    ...
    def _retrieve_offenses(self, action_result, reqheaders, reqparams):
        ...
        if phantom.is_fail(action_result.get_status()):
            if response:
                ...
                status_code = None
            self.save_progress(
                f"For alternate ingestion workflow of fetching offenses, REST call failed: {action_result.get_status()}\nResponse code: {status_code}"
            )
            return action_result.get_status()
        ...
        if response.status_code != 200:
            ...
            if "json" in response.headers.get("Content-Type", ""):
                ...
                status_message = f"{QRADAR_ERROR_LIST_OFFENSES_API_FAILED}. HTTP status_code: {response.status_code}, reason: {response.reason}"
            self.save_progress(f"Rest call error: {status_message}\nResponse code: {response.status_code}")
            return action_result.set_status(phantom.APP_ERROR, status_message)
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            self.save_progress(f"Unable to parse response as a valid JSON: {error_message}")
    ...
    def _report_back(self, new_offenses, runs):
        ...
        try:
            self.save_progress(
                "Run {}: downloaded ({}) earliest offense [ id ({}) start_time ({}) ] latest offense [ id ({}) start_time ({}) ]".format(
                    runs,
                    len(new_offenses),
                    offenses_bytime[0]["id"],
                    self._utcctime(offenses_bytime[0]["start_time"]),
    ...
    def _ingest_collected_artifacts(self, offense_id):
        ...
        self.save_progress(f"Started artifacts creation for the collected events and offenses for offense id - {offense_id}")
        for i in range(0, len(self._artifacts_list), QRADAR_DEFAULT_ARTIFACT_CHUNK_SIZE):
            ...
            self.send_progress(f"Ingesting {i + 1}-{j} artifacts, Status: In-progress")

            ret_val, create_artifact_msg, _ = self.save_artifacts(self._artifacts_list[i:j])
            if phantom.is_fail(ret_val):
                self.save_progress(f"Error occurred while artifact creation for the offense ID: {offense_id}. Error: {create_artifact_msg}")
                self.send_progress(f"Ingesting {i + 1}-{j} artifacts, Status: Failed")
                self.save_progress(" ")
                continue

            self.send_progress(f"Ingesting {i + 1}-{j} artifacts, Status: Complete")
            self.save_progress(" ")

        self.save_progress(f"Artifact ingestion successful for offense id - {offense_id}")
    ...
    def _on_poll(self, param):
        ...
        if config.get("cef_event_map", None) is not None:
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                action_result.set_status(phantom.APP_ERROR, f"Optional CEF event map is not valid JSON: {error_message}")
                return action_result.get_status()
        ...
        if phantom.is_fail(self._list_offenses(param, offenses_action_result)):
            ...
            self.debug_print(f"message: {offenses_action_result.get_message()}")
        ...
        for i, offense in enumerate(offenses):
            ...
            self.save_progress(f"Saving Container # {i}")
            ret_val, message, container_id = self.save_container(container)
            self.debug_print(f"Save container returns, ret_val: {ret_val}, message: {message}, id: {container_id}")

            self.save_progress(f"Save container returns, ret_val: {ret_val}, message: {message}, id: {container_id}")

            # if there seems an error in saving container, continue from the next offense
            if phantom.is_fail(ret_val):
                error_message = f"Error occurred while container creation for Offense ID: {offense_id}. Error: {message}"
                self.debug_print(error_message)
                if message and (
                    TENANT_NOT_FOUND_4_8.format(tid=param.get("tenant_id")) in message
                    or TENANT_NOT_FOUND_4_5.format(tid=param.get("tenant_id")) in message
                ):
            ...
            if message == "Duplicate container found":
                ...
                self.debug_print(f"Add_to_resolved: {add_to_resolved}, status: {statusOfContainer}, container_id: {container_id}")
            ...
            if self._container_only:
                ...
                if phantom.is_fail(artifacts_creation_status):
                    ...
                    self.debug_print(
                        f"Error while creating artifacts for the Offense ID: {offense_id} and Container ID {container_id}. Error: {artifacts_creation_msg}"
                    )
            ...
            if self._max_events_per_offense:
                ...
                self.save_progress(f"container id({container_id})'s artifact count : {container_artifact_count}")

                if container_artifact_count >= self._max_events_per_offense:
                    self.save_progress(
                        f"container id({container_id})'s artifact count({container_artifact_count}) is more than maximum accumulated artifacts count, "
                        "phantom won't collect artifacts"
                    )
                    continue
            ...
            self.save_progress(
                f"Back-dating the event fetching start time relative to offense start time by {self._events_ingest_start_time} minutes"
            )
            self.save_progress(
                "The modified initial epoch is: {} for fetching the events for the offense ID: {}".format(
    ...
    def _list_offenses(self, param, action_result=None):
        ...
        try:
            if (param.get("end_time") and not str(param.get("end_time")).isdigit()) or param.get("end_time") == 0:
                return action_result.set_status(
                    phantom.APP_ERROR, "Please provide valid non-zero epoch value (milliseconds) in the 'end_time' parameter"
                )
        except Exception:
            return action_result.set_status(
                phantom.APP_ERROR, "Please provide valid non-zero epoch value (milliseconds) in the 'end_time' parameter"
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(
                phantom.APP_ERROR, f"Error occurred while formation of 'start_time_msecs' for fetching the offenses. Error: {error_message}"
            )
        ...
        if (
            self._get_tz_str_from_epoch("start_time_msecs", start_time_msecs, action_result) is None
            or self._get_tz_str_from_epoch("end_time_msecs", end_time_msecs, action_result) is None
        ):
            self.debug_print(
                f"Error occurred in tz_str conversion from epoch for 'start_time_msecs' and 'end_time_msecs'. Error: {action_result.get_message()}"
            )
        else:
            self.save_progress(
                "Getting offenses data from {} to {}".format(
                    self._get_tz_str_from_epoch("start_time_msecs", start_time_msecs, action_result),
        ...
        if self._is_on_poll and self._offense_ingest_start_time > 0:
            self.save_progress(f"Original start time is: {start_time_msecs}")
            start_time_msecs = start_time_msecs - self._offense_ingest_start_time * 60 * 1000
            self.save_progress(f"Back-dating the offense start time by {self._offense_ingest_start_time} minutes")
            self.save_progress(f"The modified start_time is: {start_time_msecs}")
        ...
        if offense_ids != "None":
            ...
            for x in offense_ids:
                try:
                    if len(x.strip()) > 0 and int(x.strip()) >= 0:
                        offense_id_list.append(f"id={int(x.strip())}")
                except Exception:
                    self.debug_print(f"The provided offense: {x} is not valid")
        ...
        while True:
            ...
            if ingest_only_open:
                ...
                self.save_progress("Updated filter due to 'Ingest only open offenses' being True is:  {}".format(params["filter"]))
            ...
            if not (200 <= response.status_code <= 399):
                ...
                if "json" in response.headers.get("Content-Type", ""):
                    ...
                    status_message = (
                        f"{QRADAR_ERROR_LIST_OFFENSES_API_FAILED}. HTTP status_code: {response.status_code}, reason: {response.reason}"
                    )
                return action_result.set_status(phantom.APP_ERROR, status_message)
            ...
            try:
                ...
                error_message = self._get_error_message_from_exception(e)
                self.debug_print("Unable to parse response of 'call_api' for fetching offenses as a valid JSON", error_message)
                return action_result.set_status(
                    phantom.APP_ERROR, "Unable to parse response of 'call_api' for fetching offenses as a valid JSON"
                )
    ...
    def _handle_list_closing_reasons(self, param):
        ...
        if closing_reasons_response.status_code != 200:
            ...
            if "json" in closing_reasons_response.headers.get("Content-Type", ""):
                ...
                status_message = "{}. HTTP status_code: {}, reason: {}".format(
    ...
    def _get_rule_info(self, param):
        ...
        if phantom.is_fail(ret_val):
            return action_result.get_status()
        get_rule_info_response = self._call_api(f"analytics/rules/{rule_id}", "get", action_result, params=None, headers=None)
    ...
    def _handle_ariel_query(self, ariel_query, action_result, obj_result_key=None, offense_id=None, count=None):
        if obj_result_key:
            self.save_progress("Executing ariel query to get {0} {1}", obj_result_key, "" if (not offense_id) else f"for offense: {offense_id}")
        else:
        ...
        if phantom.is_fail(action_result.get_status()):
            ...
            return action_result.set_status(
                phantom.APP_ERROR,
                f"Error occurred while fetching events for the offense ID: {offense_id}. Response code: {None if response is None else response.status_code}. Response text: {response_text}",
            )
        ...
        while not got_error and response_json.get("status") != "COMPLETED":
            ...
            response = self._call_api(f"{QRADAR_ARIEL_SEARCH_ENDPOINT}/{search_id}", "get", action_result, send_progress=False)
        ...
        if self.get_action_identifier() == "run_query":
            ...
            self.save_progress(f"Fetching the events in chunks of {QRADAR_QUERY_HIGH_RANGE}")
            i = 0
            headers = dict()
            to_stop_fetch = 0
            while True:
    ...
    def _fetch_search_results(self, action_result, offense_id, search_id, obj_result_key, headers=None):
        if (
            (self._is_on_poll or self.get_action_identifier() == "offense_details") and self._container_id and self._offense_details
        ) or self.get_action_identifier() == "get_flows":
            local_events_list = list()

        response = self._call_api(f"{QRADAR_ARIEL_SEARCH_ENDPOINT}/{search_id}/results", "get", action_result, headers=headers)
        ...
        if response.status_code != 200:
            ...
            if "json" in response.headers.get("Content-Type", ""):
                ...
                response_text = response.text
                status_message = "{}. HTTP status_code: {}, reason: {}".format(
                    QRADAR_ERROR_ARIEL_QUERY_RESULTS_FAILED, response.status_code, response_text if response_text else "Unknown error occurred."
                )
            return action_result.set_status(phantom.APP_ERROR, status_message)
        ...
        if obj_result_key:
            ...
            if obj_result_key not in response_json:
                return action_result.set_status(phantom.APP_ERROR, f"Response JSON does not contain '{obj_result_key}' key")
            ...
            self.save_progress(f"Ariel query retrieved {len(objs)} {obj_result_key} for offense {offense_id}")
            if len(objs) > 0 and "starttime" in objs[0]:
                try:
                    self.save_progress(
                        "Ariel query retrieved {} {} for offense {}; starttime of earliest ({}) latest ({})".format(
    ...
    def _validate_times(self, param, action_result):
        if phantom.APP_JSON_START_TIME in param:
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR, f"Invalid {phantom.APP_JSON_START_TIME}")
        ...
        if phantom.APP_JSON_END_TIME in param:
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR, f"Invalid {phantom.APP_JSON_END_TIME}")
    ...
    def _get_tz_str_from_epoch(self, name, epoch_milli, action_result):
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            action_result.set_status(
                phantom.APP_ERROR, f"Error occurred while converting epoch value of '{name}' to datetime string. Error: {error_message}"
            )
            return None
    ...
    def _get_events(self, param, action_result=None):
        ...
        try:
            if param.get("start_time") and not str(param.get("start_time")).isdigit():
                return action_result.set_status(
                    phantom.APP_ERROR, "Please provide valid epoch value (milliseconds) in the 'start_time' parameter"
                )
        except Exception:
            return action_result.set_status(
                phantom.APP_ERROR, "Please provide valid non-zero epoch value (milliseconds) in the 'start_time' parameter"
            )
        ...
        if offense_id:
            ...
            if self._has_offense:
                where_clause += f" hasOffense='true' and InOffense({offense_id})"
            else:
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(
                phantom.APP_ERROR, f"Error occurred while formation of 'start_time_msecs' for fetching the events. Error: {error_message}"
            )
        ...
        if self._is_on_poll and self._event_ingest_end_time > 0:
            self.save_progress(f"Original end_time_msecs is: {end_time_msecs} and start_time_msecs is : {start_time_msecs}")
            self.save_progress(f"After-dating the event end time by {self._event_ingest_end_time} minutes")
            end_time_msecs = start_time_msecs + self._event_ingest_end_time * 60 * 1000
            if end_time_msecs > curr_epoch_msecs:
                self.debug_print(
                    "The modified end_time_msecs to fetch events exceeded the current time so the current time is taken as end_time_msecs"
                )
                end_time_msecs = curr_epoch_msecs
            self.save_progress(f"The modified end_time_msecs is: {end_time_msecs}")
        ...
        where_clause += f" starttime >= {start_time_msecs} and starttime <= {end_time_msecs}"
        ...
        where_clause += " START '{}'".format(self._get_tz_str_from_epoch("start_time_msecs", start_time_msecs, action_result))
        where_clause += " STOP '{}'".format(self._get_tz_str_from_epoch("end_time_msecs", end_time_msecs, action_result))
        ...
        if self._use_alt_ariel_query and where_clause.startswith("ORDER BY"):
            ...
            ariel_query = f"{ariel_query} where {where_clause}"
        ...
        try:
            ...
            self.debug_print(
                f"Fetching {QRADAR_QUERY_HIGH_RANGE} events by default due to failure in fetching the value of the LIMIT value from the query string"
            )

        self.save_progress(f"Sending the value {final_count} as count to finally fetch the elements using the ariel query")
        self.debug_print(f"Sending the value {final_count} as count to finally fetch the elements using the ariel query")

        self.debug_print(f"Ariel query for fetching events: {ariel_query}")
        self.save_progress(f"Ariel query for fetching events: {ariel_query}")

        ret_val = self._handle_ariel_query(ariel_query, action_result, "events", offense_id, count=final_count)

        if phantom.is_fail(ret_val):
            self.debug_print(
                f"Fetching events failed with the ariel query: {ariel_query} and offense ID: {offense_id}. Error message: {action_result.get_message()}"
            )
            return action_result.get_status()
    ...
    def _run_query(self, param):
        ...
        if phantom.is_fail(ret_val):
            self.debug_print(f"Execution of the ariel query: {query}. Error message: {action_result.get_message()}")
            return action_result.get_status()
    ...
    def _get_flows(self, param):
        ...
        if ip_to_query:
            if len(where_clause):
                where_clause += " and"
            where_clause += f" (sourceip='{ip_to_query}' or destinationip='{ip_to_query}')"
            # Update the parameter
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(
                phantom.APP_ERROR, f"Error occurred while formation of 'start_time_msecs' for fetching the flows. Error: {error_message}"
            )
        ...
        if phantom.is_fail(ret_val):
            self.debug_print(f"Fetching flows failed with the ariel query: {ariel_query}. Error message: {action_result.get_message()}")
            return action_result.get_status()
    ...
    def _handle_offense_details(self, param):
        ...
        response = self._call_api(f"siem/offenses/{offense_id}", "get", action_result)
        ...
        if response.status_code != 200:
            ...
            if "json" in response.headers.get("Content-Type", ""):
                ...
                response_text = response.text
                status_message = "{}. HTTP status_code: {}, reason: {}".format(
                    QRADAR_ERROR_GET_OFFENSE_DETAIL_API_FAILED,
                    response.status_code,
                    response_text if response_text else "Unknown error occurred.",
                )
            return action_result.set_status(phantom.APP_ERROR, status_message)
    ...
    def _handle_close_offense(self, param):
        ...
        response = self._call_api(f"siem/offenses/{offense_id}", "post", action_result, params=params)
    ...
    def _alt_manage_ingestion(self, param):
        ...
        if not operation == "set last saved events ingest time" and offense_id:
            return action_result.set_status(
                phantom.APP_ERROR, "Offense ID is required only for the operation 'set last saved events ingest time'"
            )
        ...
        if operation == "delete last saved ingestion time data":
            ...
            if not datestring:
                return action_result.set_status(
                    phantom.APP_ERROR, "The 'datetime' field must be provided for the operation 'set last saved offense ingest time'"
                )
            try:
            ...
            if not offense_id or not datestring:
                return action_result.set_status(
                    phantom.APP_ERROR,
                    "The 'offense_id' and 'datetime' fields must be provided for the operation 'set last saved events ingest time'",
                )
        ...
        if last_ingested_events_ingest_time_as_datetime:
            last_ingested_events_ingest_time_as_datetime_str = ", ".join(
                ["=".join([f"Offense ID_{key}", val]) for key, val in list(last_ingested_events_ingest_time_as_datetime.items())]
            )

        try:
            action_result.update_summary(
                {
                    "last_saved_offense_ingest_time": self._utcctime(last_saved_offense_ingest_time)
                    if last_saved_offense_ingest_time or last_saved_offense_ingest_time == 0
                    else None,
            ...
            error_message = self._get_error_message_from_exception(e)
            self.debug_print(f"Provided time is invalid. Error: {error_message}")
            return action_result.set_status(phantom.APP_ERROR, f"Provided time is invalid. Error: {error_message}")
    ...
    def handle_exception(self, e):
        ...
        self.debug_print(f"Handle exception occurred: {traceback.format_exc()}")
        return phantom.APP_SUCCESS
